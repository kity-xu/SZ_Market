...
type TagStockStatic struct {
	...
	LlCircuShare      float64 `bson:"llCircuShare"`      // 流通盘
	LlTotalShare      float64 `bson:"llTotalShare"`      // 总股本
	...
	NAVPS             float64 `bson:"nAVPS"`             // 每股净值
	LlMainIncoming    float64 `bson:"llMainIncoming"`    // 主营业务收入
	LlMainProfit      float64 `bson:"llMainProfit"`      // 主营业务利润
	...
	// 后加
	NEUndisProfit      float32 `bson:"nEUndisProfit"`      // 每股未分配利润
	NFlowLiab          float32 `bson:"nFlowLiab"`          // 流动负债
	NTotalLiabilities  float32 `bson:"nTotalLiabilities"`  // 负债总计
	NTotalHolderEquity float32 `bson:"nTotalHolderEquity"` // 股东权益合计
	NCapitalReserve    float32 `bson:"nCapitalReserve"`    // 资本公积金
	NIncomeInvestments float32 `bson:"nIncomeInvestments"` // 投资收益
}
