...
type TQ_FIN_PROTTMINDIC struct {
	...
	//// 收益质量
	//// 杜邦分析
	EPSDILUTEDNEWP dbr.NullFloat64 //  每股收益_摊薄_归属母公司的...
	ROEDILUTED     dbr.NullFloat64 // 净资产收益率_摊薄_归属母公
}
...
func (this *TQ_FIN_PROTTMINDIC) GetBaseByComCode(comcode string) (*TQ_FIN_PROTTMINDIC, error) {

	var pro TQ_FIN_PROTTMINDIC

	err := this.Db.Select("EPSDILUTEDNEWP, ROEDILUTED").
		From(this.TableName).
		Where(fmt.Sprintf("COMPCODE='%v'", comcode)).
		Where("REPORTTYPE=3  AND ISVALID=1").
		Where(fmt.Sprintf("ENDDATE=(select ENDDATE FROM tq_fin_prottmindic WHERE COMPCODE='%v' AND REPORTTYPE=3  AND ISVALID=1 ORDER BY ENDDATE DESC LIMIT 1)", comcode)).
		OrderBy("ENDDATE DESC").
		LoadStruct(&pro)
	if err != nil {
