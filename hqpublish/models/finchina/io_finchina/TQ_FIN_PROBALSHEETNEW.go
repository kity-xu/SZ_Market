...
type TQ_FIN_PROBALSHEETNEW struct {
	...
	ENDDATE  dbr.NullString  //Date 	放置本次财报的截止日期
	TOTASSET dbr.NullFloat64 //ToAs  资产总计  资产合计
	TOTLIAB  dbr.NullFloat64 //TaLb   负债合计
	RIGHAGGR dbr.NullFloat64 //OESET    所有者权益（或股东权益）合计
}
...
func NewTQ_FIN_PROBALSHEETNEW() *TQ_FIN_PROBALSHEETNEW {
	return &TQ_FIN_PROBALSHEETNEW{
		Model: Model{
			TableName: finchina.TABLE_TQ_FIN_PROBALSHEETNEW,
			Db:        MyCat,
		},
	}
}
...
func (this *TQ_FIN_PROBALSHEETNEW) GetList(scode string, market string, report_data_type int, per_page int, page int) ([]TQ_FIN_PROBALSHEETNEW, error) {
