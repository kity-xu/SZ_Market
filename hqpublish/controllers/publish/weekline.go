...
func (this *WeekLine) POST(c *gin.Context) {
...
func (this *WeekLine) PostJson(c *gin.Context) {
	...
	dlines, length, err := publish.NewWeekLine().GetWeekLine(request.SID, request.Begin)
	if err != nil {
		logging.Error("%v", err)
		return
	}
...
func (this *WeekLine) PostPB(c *gin.Context) {
	var (
		replypb []byte
		err     error
